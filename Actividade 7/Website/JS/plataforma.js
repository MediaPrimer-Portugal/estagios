$("document").ready(function () {
    ...
    var Grid = (function () {
        ...
        minHeight, maxHeight, id) {
            ...
            self.AdquireDados(tipoWidget, ultimo, ficheiro);
        ...
        /// <summary>
        /// Pede os ficheiros JSON e após a análise, vai introduzi os dados no widget
        /// </summary>
        Grid.prototype.AdquireDados = function (tipoWidget, ultimo, ficheiro) {
            var self = this;

            // APAGAR - TESTE
            if (tipoWidget === "tabela") {
                if (ficheiro === null) {
                    ficheiro = "groupedBar";
                }
            }

            //ficheiro = prompt("Nome do ficheiro", null);

            // Adicionamos o novo widget à lista de widgets atuais
            switch (tipoWidget) {
                case "area":
                    self.listaWidgets.push(new GraficoArea("widget" + idUnico, "titulo", 20, 20, 400, 400));
                    self.listaWidgets[ultimo].setDados($.parseJSON(getDados(self.listaWidgets[ultimo], "stacked")))
                    break;
                case "barras":
                    self.listaWidgets.push(new GraficoBarras("widget" + idUnico, "titulo", 20, 20, 400, 400));
                    // APAGAR - Apenas para teste do icon loading
                    self.listaWidgets[ultimo].setDados($.parseJSON(getDados(self.listaWidgets[ultimo], "stackedBar")));
                    break;
                case "linhas":
                    self.listaWidgets.push(new GraficoLinhas("widget" + idUnico, "titulo", 20, 20, 400, 400));
                    // APAGAR - Apenas para teste do icon loading.dadosSelecionados)
                    self.listaWidgets[ultimo].setDados($.parseJSON(getDados(self.listaWidgets[ultimo], "ficheiro")).dadosSelecionados)
                    break;
                case "gauge":
                    self.listaWidgets.push(new Gauge("widget" + idUnico, "titulo", 20, 20, 400, 400));
                    break;
                case "kpi":
                    self.listaWidgets.push(new KPI("widget" + idUnico, "titulo", 20, 20, 400, 400));
                    break;
                case "tabela":
                    self.listaWidgets.push(new Tabela("widget" + idUnico, "titulo", 20, 20, 400, 400));
                    // APAGAR - Apenas para teste do icon loading
                    self.listaWidgets[ultimo].setDados($.parseJSON(getDados(self.listaWidgets[ultimo], "stacked")))
                    break;
                case "pie":
                    self.listaWidgets.push(new PieChart("widget" + idUnico, "titulo", 20, 20, 400, 400));
                    // APAGAR - Apenas para teste do icon loading
                    self.listaWidgets[ultimo].setDados($.parseJSON(getDados(self.listaWidgets[ultimo], "ficheiro")).dadosSelecionados)
                    break;
            }
        ...
        Grid.prototype.GuardaInformacao = function () {
            ...
            listaWidgets.forEach(function (item, curIndex) {
                ...
                item.widgetAltura = $elemento.attr("data-gs-height");
                item.widgetLargura = $elemento.attr("data-gs-width");
                item.widgetX = $elemento.attr("data-gs-x");
                item.widgetY = $elemento.attr("data-gs-y");
